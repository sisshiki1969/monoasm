...
struct JitMemory {
    contents: *mut u8,
    counter: usize,
    reloc: Vec<(u8, usize)>, //(relocation_size, pos_in_contents)
}
...
impl JitMemory {
    ...
    fn label(&mut self) -> Label {
        let label = Label(self.reloc.len());
        label
    }
    ...
    fn push_rd(&mut self, val: u8, r: Reg) {
        let c = self.counter;
    ...
    */
    ...
    fn pushq(&mut self, val: u64) {
        self.pushl(val as u32);
        self.pushl((val >> 32) as u32);
    }
...
impl JitMemory {
    ...
    fn jmp(&mut self, dest: Or) {
        match dest {
            Or::Imm(disp) => {
                self.push(0xe9);
                self.pushl((disp as i64 - self.counter as i64 - 4) as u32);
            }
            _ => unimplemented!(),
        }
    }

    fn jne(&mut self, dest: Or) {
        match dest {
            Or::Imm(disp) => {
                self.push(0x0f);
                self.push(0x85);
...
fn get_func_addr() -> (fn() -> i64) {
    let hello = "Hello World! Are you angry?\n";
    let mut jit: JitMemory = JitMemory::new();

    jit.movq(Or::Reg(Reg::Rdi), Or::Imm(1));
    jit.movq(Or::Reg(Reg::R8), Or::Imm(jit.contents as u64 + 256));
